...
def data_import (working_dir, region, runs):
    ...
    import pandas as pd  
    xls=pd.ExcelFile("hbvbd_model_inputs_revision.xlsx")        
    ...
    if runs == 1:
        globz=pd.read_excel(xls, "global", index_col=None).iloc[0:30, 1:3]      
       ...
       hbv_pars=pd.DataFrame(pd.read_excel(xls, level))
       hbv_pars_lb=pd.DataFrame(pd.read_excel(xls, level+"_lb"))             
